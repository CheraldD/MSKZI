...
// Обновленная функция для запуска одного теста (напрямую в Вейерштрассе)
void run_mult_test_vey(
    const std::string& x_str, const std::string& y_str,
    const std::string& k_str,
    const std::string& x_expected_str, const std::string& y_expected_str,
    const std::string& test_name)
{
    std::cout << "--- " << test_name << " (напрямую в Вейерштрассе) ---" << std::endl;

    Digit x[CD], y[CD], k[CD];
    StringToNumber(x_str, x);
    StringToNumber(y_str, y);
    StringToNumber(k_str, k);

    Digit x_res[CD], y_res[CD];
    Mult_Vey(x, y, k, x_res, y_res);

    std::string x_calculated_str = NumToStringDigit(x_res);
    std::string y_calculated_str = NumToStringDigit(y_res);
    ...
    bool success = (to_upper(x_calculated_str) == to_upper(x_expected_str)) &&
                   (to_upper(y_calculated_str) == to_upper(y_expected_str));
...
// Обновленная функция для теста через преобразование в Эдвардса
void run_mult_test_edv_from_vey(
    const std::string& x_str, const std::string& y_str,
    const std::string& k_str,
    const std::string& x_expected_str, const std::string& y_expected_str,
    const std::string& test_name)
{
    ...
    Digit x_vey[CD], y_vey[CD], k[CD];
    StringToNumber(x_str, x_vey);
    StringToNumber(y_str, y_vey);
    StringToNumber(k_str, k);
    
    edv_points p_edv = VeyToEdv(x_vey, y_vey);

    Digit result_edv_u[CD], result_edv_v[CD];
    Mult_Edv(p_edv.u, p_edv.v, k, result_edv_u, result_edv_v);
...
// Основная функция с данными в ВЕРХНЕМ РЕГИСТРЕ
void test_scalar_multiplication() {
    std::cout << "============================================================" << std::endl;
    std::cout << "          ТЕСТИРОВАНИЕ СКАЛЯРНОГО УМНОЖЕНИЯ ТОЧКИ           " << std::endl;
    std::cout << "============================================================" << std::endl << std::endl;
    ...
    std::string a1_x = "91E38443A5E82C0D880923425712B2BB658B9196932E02C78B2582FE742DAA28";
    std::string a1_y = "32879423AB1A0375895786C4BB46E9565FDE0B5344766740AF268ADB32322E5C";
    std::string k1   = "E8C6740E58D616CA220DB7DA0D9C3E19B53E86E38BF3E8747774631452EC174C";
    std::string b1_x = "715DC6983EB402D2ABDC9EF0A20F4BA8F91BB1998992FECB8D80D6A3422D0588";
    std::string b1_y = "D4E78F07D0473DE3EE38F856199B4DC9BA4A87C263BB53938EF21CCEF37203FE";
    
    std::cout << "Исходные данные:" << std::endl;
    std::cout << "  Точка A1.x: " << a1_x << std::endl;
    std::cout << "  Точка A1.y: " << a1_y << std::endl;
    std::cout << "  Коэфф. k1:  " << k1 << std::endl << std::endl;

    run_mult_test_vey(a1_x, a1_y, k1, b1_x, b1_y, "Тест B1 = A1 * k1");
    run_mult_test_edv_from_vey(a1_x, a1_y, k1, b1_x, b1_y, "Тест B1 = A1 * k1");
    ...
    std::string a2_x = "1D40C1676805F9518BE1FB4C7AE460D3608581E477B07C2D0E7E1E265A6B3347";
    std::string a2_y = "8291ACE380FD8832BACA29613AB5626C302D13348F204D727D30897A8E1F8934";
    std::string k2   = "C9CBAEEFAABC51147130FC6FA1ADBE72140E35C5911B7D54B12BEECDF5848943";
    std::string b2_x = "1DFBF03073736C14453519F06FE712A3814F954168878907E02B9700678E3DE1";
    std::string b2_y = "E8C641CCA3F7A197EBF8E4F4F85A64F0842A2B74DE1627443D29EE63E1E8CA28";

    std::cout << "Исходные данные:" << std::endl;
    std::cout << "  Точка A2.x: " << a2_x << std::endl;
    std::cout << "  Точка A2.y: " << a2_y << std::endl;
    std::cout << "  Коэфф. k2:  " << k2 << std::endl << std::endl;

    run_mult_test_vey(a2_x, a2_y, k2, b2_x, b2_y, "Тест B2 = A2 * k2");
    run_mult_test_edv_from_vey(a2_x, a2_y, k2, b2_x, b2_y, "Тест B2 = A2 * k2");

    std::cout << "============================================================" << std::endl;
    std::cout << "             ТЕСТИРОВАНИЕ УМНОЖЕНИЯ ЗАВЕРШЕНО               " << std::endl;
    std::cout << "============================================================" << std::endl;
}
void run_single_multiplication_task(
    const std::string& x_str, const std::string& y_str,
    const std::string& k_str, const std::string& point_name)
{
    std::cout << "--- Вычисление для '" << point_name << "' ---" << std::endl;
    std::cout << "  Исходная точка:" << std::endl;
    std::cout << "    x = " << x_str << std::endl;
    std::cout << "    y = " << y_str << std::endl;
    std::cout << "  Коэффициент:" << std::endl;
    std::cout << "    k = " << k_str << std::endl << std::endl;

    // --- Метод 1: Прямое вычисление на кривой Вейерштрасса ---
    Digit x[CD], y[CD], k[CD];
    StringToNumber(x_str, x);
    StringToNumber(y_str, y);
    StringToNumber(k_str, k);
    
    Digit res_x_vey[CD], res_y_vey[CD];
    Mult_Vey(x, y, k, res_x_vey, res_y_vey);

    std::cout << "  [1] Результат (метод Вейерштрасса):" << std::endl;
    std::cout << "      x' = " << NumToStringDigit(res_x_vey) << std::endl;
    std::cout << "      y' = " << NumToStringDigit(res_y_vey) << std::endl << std::endl;

    // --- Метод 2: Вычисление через преобразование в Эдвардса ---
    edv_points p_edv = VeyToEdv(x, y);
    Digit res_u_edv[CD], res_v_edv[CD];
    Mult_Edv(p_edv.u, p_edv.v, k, res_u_edv, res_v_edv);
    vey_points final_vey = EdvToVey(res_u_edv, res_v_edv);

    std::cout << "  [2] Результат (метод через Эдвардса):" << std::endl;
    std::cout << "      x' = " << NumToStringDigit(final_vey.x) << std::endl;
    std::cout << "      y' = " << NumToStringDigit(final_vey.y) << std::endl;
}


/**
 * @brief Главная функция для запуска вычислений по Варианту 20.
 */
void test_multiplication_variant_20()
{
    std::cout << "============================================================" << std::endl;
    std::cout << "        СКАЛЯРНОЕ УМНОЖЕНИЕ ТОЧЕК (ВАРИАНТ 20)      " << std::endl;
    std::cout << "============================================================" << std::endl << std::endl;
    
    // --- Данные из Варианта 20 ---
    std::string ax = "7C62F4FF84427D9C84074D745167BF0A1E1B5B2FA400836B4B991BEC29BF999F";
    std::string ay = "08EE3D6E617B30D21ACDDEAEA988A09A02E8FAE6FE1B498E2EFA951E9EB1B0F2";
    
    std::string bx = "4AFC8D2A703BD0ED2F55F741FC12AD71B8CFD2D78B70758CBF844CE79E83781F";
    std::string by = "FC5C10486F70C07CFFE06F1C64A094028A574E85AC112CEFB94A0772D2381FEC";

    std::string k = "C679A161FE4411015024048DB645D8E070C8C6C3C224BA8B4439DF97AA0DB4F9";

    // Запускаем вычисление для Точки А
    run_single_multiplication_task(ax, ay, k, "Точка А");

    std::cout << "------------------------------------------------------------" << std::endl << std::endl;
    
    // Запускаем вычисление для Точки B
    run_single_multiplication_task(bx, by, k, "Точка B");
