...
void Mult_Vey(const Digit x[], const Digit y[], const Digit k[], Digit x_res[], Digit y_res[])
{
    vey_points P_temp;
    Assig(x, P_temp.x);
    Assig(y, P_temp.y);
    ...
    Assig(g_zero, R.x);
    Assig(g_zero, R.y);
    bool is_R_at_infinity = true;

    for (int i = 0; i < 256; i++)
    {
       R = Add_Vey(R, P_temp, nullptr, nullptr);
        if ((k[i / 16] >> (i % 16)) & 1)
        {
        ...
        P_temp = Add_Vey(P_temp, P_temp, nullptr, nullptr);
    }
...
void Mult_Edv(const Digit u[], const Digit v[], const Digit k[], Digit u_res[], Digit v_res[])
{
    edv_points P_temp;
    Assig(u, P_temp.u);
    Assig(v, P_temp.v);
    ...
    edv_points R;
    Assig(g_zero, R.u); // u = 0
    Assig(g_e, R.v);    // v = 1

    bool is_R_neutral = true;
    
    Digit tempU[CD], tempV[CD];

    for (int i = 0; i < 256; i++)
    {
        Add_edv(R, P_temp, tempU, tempV);
        if ((k[i / 16] >> (i % 16)) & 1)
        {
        ...
        Add_edv(P_temp, P_temp, tempU, tempV);
        
        Assig(tempU, P_temp.u);
        Assig(tempV, P_temp.v);
    }
